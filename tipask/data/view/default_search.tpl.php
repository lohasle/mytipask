...
<? if(is_array($questionlist)) { foreach($questionlist as $question) { ?>
                    <li class="item">
                        ...
                        <div class="qa-i-ft"><span class="cate"><a href="<?=SITE_URL?>?c-<?=$question['cid']?>.html" target="_blank" text="qaresult-cate"><?=$question['category_name']?></a></span> - 提问人:<? if($question['hidden']) { ?><span>匿名</span><? } else { ?><a href="<?=SITE_URL?>?u-<?=$question['authorid']?>.html"><?=$question['author']?></a><? } ?> - <?=$question['answers']?>个回答 - 提问时间: <?=$question['format_time']?></div>
                    </li>
...
<? } } ?>
            ...
            <? } ?>            <? if($setting['xunsearch_open'] && !$tag) { ?>            <div class="mod-relation">
                <div id="rs">
                    <table>
                        <tbody>
                            <tr>
